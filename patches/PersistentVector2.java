...
public class PersistentVector2 extends APersistentVector implements IObj, IEditableCollection, IReduce, IKVReduce{
...
public final static PersistentVector2 EMPTY = new PersistentVector2(0, 5, EMPTY_NODE, new Object[]{});
...
static public PersistentVector2 adopt(Object [] items){
	return new PersistentVector2(items.length, 5, EMPTY_NODE, items);
}

static public PersistentVector2 create(IReduceInit items) {
    TransientVector ret = EMPTY.asTransient();
    items.reduce(TRANSIENT_VECTOR_CONJ, ret);
    return ret.persistent();
}

static public PersistentVector2 create(ISeq items){
    ...
    if(items != null) {  // >32, construct with array directly
        PersistentVector2 start = new PersistentVector2(32, 5, EMPTY_NODE, arr);
        TransientVector ret = start.asTransient();
        for (; items != null; items = items.next())
            ret = ret.conj(items.first());
        return ret.persistent();
    } else if(i == 32) {   // exactly 32, skip copy
        return new PersistentVector2(32, 5, EMPTY_NODE, arr);
    } else {  // <32, copy to minimum array and construct
        Object[] arr2 = new Object[i];
        System.arraycopy(arr, 0, arr2, 0, i);
        return new PersistentVector2(i, 5, EMPTY_NODE, arr2);
    }
}

static public PersistentVector2 create(List list){
    int size = list.size();
    if (size <= 32)
        return new PersistentVector2(size, 5, PersistentVector2.EMPTY_NODE, list.toArray());
...
static public PersistentVector2 create(Iterable items){
...
static public PersistentVector2 create(Object... items){
	TransientVector ret = EMPTY.asTransient();
...
PersistentVector2(int cnt, int shift, Node root, Object[] tail){
	this._meta = null;
	this.cnt = cnt;
	this.shift = shift;
	this.root = root;
...
PersistentVector2(IPersistentMap meta, int cnt, int shift, Node root, Object[] tail){
	this._meta = meta;
	this.cnt = cnt;
	this.shift = shift;
	this.root = root;
...
public PersistentVector2 assocN(int i, Object val){
	if(i >= 0 && i < cnt)
		{
		if(i >= tailoff())
			{
			...
			return new PersistentVector2(meta(), cnt, shift, root, newTail);
			}

		return new PersistentVector2(meta(), cnt, shift, doAssoc(shift, root, i, val), tail);
		}
	if(i == cnt)
		return cons(val);
	throw new IndexOutOfBoundsException();
...
public PersistentVector2 withMeta(IPersistentMap meta){
	if(meta() == meta)
		return this;
	return new PersistentVector2(meta, cnt, shift, root, tail);
}
...
public PersistentVector2 cons(Object val){
	//room in tail?
//	if(tail.length < 32)
	if(cnt - tailoff() < 32)
		{
		...
		return new PersistentVector2(meta(), cnt + 1, shift, root, newTail);
		}
	...
	return new PersistentVector2(meta(), cnt + 1, newshift, newroot, new Object[]{val});
}
...
static public final class ChunkedSeq extends ASeq implements IChunkedSeq,Counted{
	...
	public final PersistentVector2 vec;
	final Object[] node;
	final int i;
	public final int offset;

	public ChunkedSeq(PersistentVector2 vec, int i, int offset){
		this.vec = vec;
		this.i = i;
		this.offset = offset;
	...
	ChunkedSeq(IPersistentMap meta, PersistentVector2 vec, Object[] node, int i, int offset){
		super(meta);
		this.vec = vec;
		this.node = node;
		this.i = i;
	...
	ChunkedSeq(PersistentVector2 vec, Object[] node, int i, int offset){
		this.vec = vec;
		this.node = node;
		this.i = i;
		this.offset = offset;
...
public PersistentVector2 pop(){
	...
	return new PersistentVector2(meta(), cnt - 1, newshift, newroot, newtail);
}
...
static final class TransientVector extends AFn implements ITransientVector, ITransientAssociative2, Counted{
	...
	TransientVector(PersistentVector2 v){
	...
	public PersistentVector2 persistent(){
		ensureEditable();
...
//			}
		...
		return new PersistentVector2(cnt, shift, root, trimmedTail);
	}
...
static public void main(String[] args){
	if(args.length != 3)
		{
		System.err.println("Usage: PersistentVector2 size writes reads");
		return;
		}
...
//	v.setSize(size);
	...
	PersistentVector2 p = PersistentVector2.EMPTY;
...
//		mp = mp.conj(i);
	...
	System.out.println("PersistentVector2");
	rand = new Random(42);
	startTime = System.nanoTime();
	long tp = 0;

//	PersistentVector2 oldp = p;
...
//		p = p.pop();
	...
	p = (PersistentVector2) mp.immutable();
	//mp.pop();  //should fail
	for(int i = 0; i < size / 2; i++)
		{
		tp += (Integer) p.nth(i);
